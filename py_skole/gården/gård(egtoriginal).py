...
class Gård:
    ...
    def hvertsekund(self):
        while True:
            time.sleep(0.05) #dager per sekund
            for i in self.felt:
                i.oppdater()
            gårder[self] = self
            savegård()
...
class Felt:
    ...
    def status(self):
        ...
        for i in self.produsert:
            if type(self.produsert[i]) is list:
                print(f'{i}: {self.produsert[i][0]}{self.produsert[i][1]}')
            else:
            ...
            for y in self.innhold[i]:
                ...
                if o:
                    ...
                    if o[0] == 'slaktes':
                        self.slaktdyr(o[1])
                    if o[0] == 'produser':
##                        print(self.produsert)
                        dictadd(self.produsert, o[1].produserer[1], o[1].produserer[0])
...
class Avlingsfelt(Felt):
    ...
    def høst(self, plante):
        ...
        if k:
            for i in k:
                dictadd(self.produsert, i.plante[0], i.høst())
        else:
...
class Dyrefelt(Felt):
    ...
    def slaktdyr(self, dyr):
        ...
        dictadd(self.produsert, 'kjøtt', Dyr.dyreattributer[dyr.dyr[0]]['kjøtt'])
##        if dyr.dyr[0] in self.produsert:
##            self.produsert[dyr.dyr[0]]['kjøtt'][0] += Dyr.dyreattributer[dyr.dyr[0]]['kjøtt']
##        else:
##            self.produsert[dyr.dyr[0]] = {'kjøtt': [Dyr.dyreattributer[dyr.dyr[0]]['kjøtt'], 'kg']}
...
class Dyr:
    dyreattributer = {
            'ku': {'føder': '2år', 'slaktes' : '5år', 'plass': 2, 'kjøtt': 300, 'produseres': (25, 'Melk i liter')},
            'høne': {'føder' : '126dager', 'slaktes' : '2år', 'plass': 0.5, 'kjøtt': 2.5, 'produseres': (1, 'egg')},
    ...
    def __init__(self, dyr):
        ...
        self.produserer = Dyr.dyreattributer[self.dyr[0]]['produseres'] if 'produseres' in Dyr.dyreattributer[self.dyr[0]] else []
    ...
    def oppdater(self):
        ...
        if self.status == 'føder' and self.produserer:
            return ['produser', self]
